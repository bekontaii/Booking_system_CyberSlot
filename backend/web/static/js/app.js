...
function initBooking() {
  ...
  const loadPCsByClub = async (clubID) => {
    ...
    if (!result.ok) {
      setMessage(message, result.data.error || "Не удалось загрузить компьютеры", "error");
      return;
    }
  ...
  const loadBookings = async () => {
    ...
    if (!result.ok) {
      setMessage(message, result.data.error || "Не удалось загрузить бронирования", "error");
      return;
    }
